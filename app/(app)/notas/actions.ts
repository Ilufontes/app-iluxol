...
export async function buscarClientes(termino: string) {
  ...
  const { data, error } = await supabase
    .from('clientes')
    .select('id, nombre, telefono, telefono2, email, otros_datos, lpd_firmado, domicilios ( id, direccion, zona, datos_vivienda, municipios ( nombre ) )')
...
export async function crearNota(datos: DatosNota) {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      ...
      domicilios ( id, direccion, zona, datos_vivienda, municipios ( nombre ) ),
      tipo_notas ( nombre ),
